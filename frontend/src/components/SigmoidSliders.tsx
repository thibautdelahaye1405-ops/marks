...
export interface SigmoidTraderParams {
  sigma_atm: number;   // ATM Vol Level
  s_atm: number;       // ATM Skew
  k_atm: number;       // ATM Kurtosis
  w_p: number;         // Put Wing Slope
  w_c: number;         // Call Wing Slope
  sigma_min: number;   // Min Vol Level
}

interface Props {
  values: SigmoidTraderParams;
  baseValues?: SigmoidTraderParams;  // fitted values — sliders center around these
  onChange: (params: SigmoidTraderParams) => void;
  onReset?: () => void;
}
...
const PARAM_DEFS: ParamDef[] = [
  { key: "sigma_atm", label: "ATM Vol (\u03C3_ATM)", hardMin: 0.01,  hardMax: 3.0, halfWidth: 0.05, nSteps: 200, fmt: (v) => (v * 100).toFixed(1) + "%" },
  { key: "s_atm",     label: "ATM Skew (S)",    hardMin: -5.0,  hardMax: 5.0, halfWidth: 0.05, nSteps: 200, fmt: (v) => v.toFixed(4) },
  { key: "k_atm",     label: "Kurtosis (K)",    hardMin: 0.0,   hardMax: 5.0, halfWidth: 0.05, nSteps: 200, fmt: (v) => v.toFixed(4) },
  { key: "w_p",       label: "Put Wing (W_P)",  hardMin: 0.001, hardMax: 5.0, halfWidth: 0.05, nSteps: 200, fmt: (v) => v.toFixed(4) },
  { key: "w_c",       label: "Call Wing (W_C)", hardMin: 0.001, hardMax: 5.0, halfWidth: 0.05, nSteps: 200, fmt: (v) => v.toFixed(4) },
  { key: "sigma_min", label: "Min Vol (\u03C3_min)", hardMin: 0.005, hardMax: 3.0, halfWidth: 0.05, nSteps: 200, fmt: (v) => (v * 100).toFixed(1) + "%" },
];

export default function SigmoidSliders({ values, baseValues, onChange, onReset }: Props) {
  ...
  const handleSlider = useCallback((key: keyof SigmoidTraderParams, val: number) => {
